#!/usr/bin/env bash
# Corrected re-run: in-city (block split) + deployment model + downstream + paper.
# Keeps existing kerala_loco.json (already good).
cd "/c/Users/vivek/OneDrive/Desktop/CVVP paper 2" || exit 1
L=results/logs
run () { echo "STAGE $1 START"; bash -c "$2" > "$L/$1.log" 2>&1; echo "STAGE $1 END rc=$?"; }

run kerala_incity_full "python code/training/train_kerala.py --mode incity full --models vit_s resnet50 effb0 --epochs 30 --bs 128"
